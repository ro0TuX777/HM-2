# dm_devices_routes.py

from flask import Blueprint, jsonify, request
from database import get_db

# Blueprint for device management routes
dm_devices_routes_bp = Blueprint('dm_devices_routes', __name__)

# Route to get all devices
@dm_devices_routes_bp.route('/devices', methods=['GET'])
def get_all_devices():
    db = get_db()
...
@dm_devices_routes_bp.route('/devices', methods=['POST'])
def add_new_device():
...
@dm_devices_routes_bp.route('/devices/position', methods=['PUT'])
...
@dm_devices_routes_bp.route('/devices', methods=['DELETE'])
def delete_device():
    data = request.get_json()
    name = data.get('name')
...
@dm_devices_routes_bp.route('/devices/<name>', methods=['GET'])
